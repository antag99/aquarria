...
//		actualSize = windowSize;
	...
	public void decompress(ByteBuffer input, int inputLength,
			ByteBuffer output, int outputLength) {
...
//				System.out.println("Done decompressing");
	...
	private void decompressBlock(ByteBuffer input, int inputLength,
			ByteBuffer output, int outputLength) {
...
//						System.out.println("I= " + i + ", J=" + j);
				...
				case Verbatim:
					...
					if(mainTree.getLength()[0xE8] != 0) intelStarted = true;
...
//		outData.Write(window, start_window_pos, outLen);
		...
		if((framesRead++ < 32768) && intelFileSize != 0)
		{
			...
			else
			{
				...
				{
					if(output.get() != 0xE8) { curpos++; continue; }
				}
			}
...
//		                m_state.PRETREE_len, m_state.PRETREE_table);
		...
		for(x = first; x < last;)
		{
			...
			if(z == 17)
			{
				y = buffer.readBits(4); y += 4;
				while(y-- != 0) lens[x++] = 0;
			}
			else if(z == 18)
			{
				y = buffer.readBits(5); y += 20;
				while(y-- != 0) lens[x++] = 0;
			}
			else if(z == 19)
			{
				y = buffer.readBits(1); y += 4;
				z = preTree.readHuffSym(buffer);
...
class HuffTable {
	...
	public void makeDecodeTable() {
		...
		if(pos != table_mask)
		{
			...
			for(sym = (short)pos; sym < table_mask; sym++) table[sym] = 0;
		...
		for(sym = 0; sym < maxSymbols; sym++) if(length[sym] != 0) return;// 1;
	}
